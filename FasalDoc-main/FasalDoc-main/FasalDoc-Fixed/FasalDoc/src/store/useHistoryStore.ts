...
export const useHistoryStore = create<HistoryState>((set) => ({
  ...
  addScan: async (record: ScanRecord) => {
    try {
      const { insertScan } = require('../services/offlineDB');
      insertScan(record);
      set((state) => ({ scans: [record, ...state.scans] }));
    } catch (err) {
      console.error('[HistoryStore] addScan failed:', err);
      // Still update in-memory even if DB fails
      set((state) => ({ scans: [record, ...state.scans] }));
    }
  },
  ...
  updateStatus: async (id: string, status: ScanStatus) => {
    try {
      const { updateScanStatus } = require('../services/offlineDB');
  ...
  loadFromDB: async () => {
    set({ isLoading: true });
    try {
      const { getAllScans } = require('../services/offlineDB');
