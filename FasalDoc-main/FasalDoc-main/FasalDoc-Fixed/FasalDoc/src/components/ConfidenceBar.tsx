...
const ConfidenceBar: React.FC<Props> = ({ value, showLabel = true }) => {
  const scheme = useColorScheme();
  const C = getColors(scheme);
  const { t } = useTranslation();
  const width = useSharedValue(0);

  const barColor = value >= 80 ? C.PRIMARY_GREEN : value >= 60 ? C.AMBER : C.RED;
...
const styles = StyleSheet.create({
  ...
  label: { ...Fonts.caption },
  value: { ...Fonts.caption, fontWeight: '500' },
